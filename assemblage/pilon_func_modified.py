...
def polish_with_pilon_multiple_rounds(graph, insert_size_graph, args, polish_dir,
    ...
    for i in range(settings.MAX_PILON_POLISH_COUNT):
        ...
        ### DEBUT MODIF - 1/2
        if change_count == 0:
          #separation des traitements des 'bases' et 'tous types' de modifications par Pilon
...
def polish_with_pilon(graph, args, polish_dir, insert_size_1st, insert_size_99th, round_num,
    ...
    ### DEBUT MODIF - 2/2
    if using_paired_reads and fix_type != 'all' : #traitement en paired-end pour les modifs en 'bases'
        pilon_command += ['--frags', paired_bam_filename]
    else:
        #fix_type==all 
        #traitement en comme si les données n'étaient pas paired-end pour les modifs de types 'all'
        # Autrement 'all' en paired BUG au sein de Pilon
        pilon_command += ['--unpaired', paired_bam_filename]
    ### FIN MODIF - 2/2    
